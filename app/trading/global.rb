...
class Global
  ...
  def default_ticker
    { low: ZERO, high: ZERO, last: ZERO, volume: ZERO, quote_volume: ZERO }
  end
 ...
 # Use to calculate quote volume. 
def h24_funds

    cache_fetch(:h24_funds) do

      Trade.where(market_id: market_id).h24.sum(:funds) || ZERO
   end
end
